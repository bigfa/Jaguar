...
class jaguarBase
{
    ...
    public function __construct()
    {
        ...
        register_nav_menu('jaguar', 'jaguar');
        register_nav_menu('jaguar_footer', 'jaguar_footer');
        ...
        if ($jaguarSetting->get_setting('toc'))
            add_filter('the_content', array($this, 'jaguar_toc'));
        if ($jaguarSetting->get_setting('gravatar_proxy'))
        ...
        if ($jaguarSetting->get_setting('exclude_status'))
            add_filter('pre_get_posts', array($this, 'exclude_post_format'));
        if ($jaguarSetting->get_setting('image_zoom'))
            add_filter('the_content', array($this, 'image_zoom'));

        if ($jaguarSetting->get_setting('rss_tag'))
    ...
    {
        global $jaguarSetting;
        echo $jaguarSetting->get_setting('rss_tag');
    }

    function add_figure_has_by_image_slug($content)
    {
        // get a;; figures
        $pattern = "/<figure.*?>(.*?)<\/figure>/i";
        preg_match_all($pattern, $content, $matches);
        if ($matches && isset($matches[0]) && is_array($matches[0])) {
            foreach ($matches[0] as $figure) {
                // get the image src
                preg_match('/<img.*?src=[\'"]([^\'"]+)[\'"].*?\/?>/i', $figure, $img_matches);
                if (isset($img_matches[1])) {
                    $image_src = $img_matches[1];
                    // get the image slug
                    $image_slug = basename($image_src);
                    // remove the file extension
                    $image_slug = preg_replace('/\.[^.]+$/', '', $image_slug);

                    // add data-by attribute to the figure
                    $figure_with_by = str_replace('<figure', '<figure data-id="' . $image_slug . '"', $figure);
                    // replace the figure in the content
                    $content = str_replace($figure, $figure_with_by, $content);
                }
            }
        }
        return $content;
    ...
    function add_category_cover_form_item($category)
    {
        ...
        <table class="form-table">
            <tr class="form-field">
                <th scope="row" valign="top"><label for="_category_cover"><?php _e('Cover', 'Jaguar'); ?></label></th>
                <td><input name="_category_cover" id="_category_cover" type="text" size="40" aria-required="false" value="<?php echo $cover; ?>" class="regular-text ltr" />
                    <p class="description"><button id="upload-categoryCover" class="button"><?php _e('Upload', 'Jaguar'); ?></button></p>
                    <p class="description"><?php _e('Category cover url.', 'Jaguar'); ?></p>
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row"><?php _e('Card Template', 'Jaguar'); ?></th>
                <td>
                    <fieldset>
                        <legend class="screen-reader-text"><span>
                                <?php _e('Card Template', 'Jaguar'); ?></span></legend><label for="_category_card">
                            <input name="_category_card" type="checkbox" id="_category_card" value="1" <?php if ($card) echo 'checked' ?>>
                            <?php _e('Use Card Template', 'Jaguar'); ?></label>
                    </fieldset>
                </td>
            </tr>
        </table>
<?php }
    ...
    {
        global $jaguarSetting;
        $url = str_replace(array("www.gravatar.com", "cn.gravatar.com", "0.gravatar.com", "1.gravatar.com", "2.gravatar.com", "secure.gravatar.com"), $jaguarSetting->get_setting('gravatar_proxy'), $url);
        return $url;
    }

    function jaguar_toc($content)
    {
        global $jaguarSetting;
        $toc_start = $jaguarSetting->get_setting('toc_start') ? $jaguarSetting->get_setting('toc_start') : 3;
    ...
    function head_output()
    {
        ...
        if (is_singular()) {
            ...
            if (get_post_meta($ID, "_desription", true)) {
                ...
                $description = $post->post_title . '，' . __('author', 'Jaguar') . ':' . get_the_author_meta('nickname', $author) . '，' . __('published on', 'Jaguar') . get_the_date('Y-m-d');
            }
            echo '<meta name="description" content="' . $description . '">';
            $ogmeta .= '<meta property="og:image" content="' . jaguar_get_background_image($ID) . '">';
            ...
            $twitter_meta .= '<meta name="twitter:image:src" content="' . jaguar_get_background_image($post->ID) . '">';
            ...
            if (is_home()) {
                $description = $jaguarSetting->get_setting('description');
            } elseif (is_category()) {
            ...
            if ($jaguarSetting->get_setting('og_default_thumb')) {
                $ogmeta .= '<meta property="og:image" content="' . $jaguarSetting->get_setting('og_default_thumb') . '">';
            }
    ...
    function widgets_init()
    {
        ...
        register_sidebar(array(
            'name'          => __('Homepage Top', 'Jaguar'),
            'id'            => 'topbar',
            'description'   => __('Homepage Top', 'Jaguar'),
        ...
        register_sidebar(array(
            'name'          => __('Homepage Bottom', 'Jaguar'),
            'id'            => 'footerbar',
            'description'   => __('Homepage Bottom', 'Jaguar'),
            'before_widget' => '<aside id="%1$s" class="widget %2$s">',
            'after_widget'  => '</aside>',
            'before_title'  => '<h3 class="heading-title">',
            'after_title'   => '</h3>',
        ...
        register_sidebar(array(
            'name'          => __('Single Pgae Bottom', 'Jaguar'),
            'id'            => 'singlefooterbar',
            'description'   => __('Single Pgae Bottom', 'Jaguar'),
            'before_widget' => '<aside id="%1$s" class="widget %2$s">',
            'after_widget'  => '</aside>',
            'before_title'  => '<h3 class="heading-title">',
            'after_title'   => '</h3>',
        ...
        if (isset($_GET['taxonomy']) && $_GET['taxonomy'] == 'category') {
            ...
            wp_localize_script(
                ...
                [
                    ...
                    'upload_button' => __('Set Category Image', 'Jaguar'),
                ]
            );
        }
    }
    ...
    function enqueue_styles()
    {
        ...
        wp_enqueue_style('jaguar-style', get_template_directory_uri() . '/build/css/misc.css', array(), JAGUAR_VERSION, 'all');
        if ($jaguarSetting->get_setting('css')) {
            wp_add_inline_style('jaguar-style', $jaguarSetting->get_setting('css'));
        }
        if ($jaguarSetting->get_setting('disable_block_css')) {
    ...
    {
        global $jaguarSetting;
        wp_enqueue_script('jaguar-script', get_template_directory_uri() . '/build/js/ts.js', [], JAGUAR_VERSION, true);
        wp_localize_script(
            'jaguar-script',
            'obvInit',
            [
                ...
                'restfulBase' => esc_url_raw(rest_url()),
                'nonce' => wp_create_nonce('wp_rest'),
                'darkmode' => !!$jaguarSetting->get_setting('darkmode'),
                'version' => JAGUAR_VERSION,
        ...
        if ($jaguarSetting->get_setting('javascript')) {
            wp_add_inline_script('jaguar-script', $jaguarSetting->get_setting('javascript'));
        }
        if (is_singular()) wp_enqueue_script("comment-reply");
    }
}
global $jaguarBase;
$jaguarBase = new jaguarBase();
