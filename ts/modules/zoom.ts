interface Window {
    findParent(t: any, e: any, n: any): any;
}
...
class imgZoom {
    ...
    renderNav() {
        ...
        const navElement = document.querySelector('.image--nav');
        if (navElement) {
            navElement.innerHTML = nav;
        }
        if (this.currentIndex === 0) {
            document.querySelector('.mfp-arrow-left')?.classList.add('disabled');
        } else {
            document.querySelector('.mfp-arrow-left')?.classList.remove('disabled');
        }
        if (this.currentIndex === this.images.length - 1) {
            document.querySelector('.mfp-arrow-right')?.classList.add('disabled');
        } else {
            document.querySelector('.mfp-arrow-right')?.classList.remove('disabled');
        }
    }
    ...
    loadImage(o: any) {
        let s = new Image();
        const rippleElement = document.querySelector('.lds-ripple');
        if (rippleElement) {
            (rippleElement as HTMLElement).style.display = 'inline-block';
        }
        s.onload = () => {
            var t = s.width,
                e = s.height,
            ...
            const overlayImgWrap = document.querySelector('.overlay-img-wrap');
            if (overlayImgWrap) {
                overlayImgWrap.classList.add('is-finieshed');
            }
            const rippleElement = document.querySelector('.lds-ripple');
            if (rippleElement) {
                (rippleElement as HTMLElement).style.display = 'none';
            }
        };
        s.src = o;
    }
